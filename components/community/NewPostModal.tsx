...
import { platforms, roles } from '@/data/ai-guide';

const PREDEFINED_TOOLS = platforms.map((platform) => platform.name);
const DISCIPLINE_OPTIONS = roles.filter((role) => role !== 'All Roles');
...
export function NewPostModal({ isOpen, onClose, onSubmit, initialData, parentTitle }: NewPostModalProps) {
  ...
  useEffect(() => {
    if (isOpen && initialData?.tool && !PREDEFINED_TOOLS.includes(initialData.tool)) {
